...
def partition_pc(pc_file, block_size, keep_size, normalize, input_dir, output_dir, total_blocks, tqdm_handle):
    ...
    points = pc.points# dataframe 就是个excel表 xyzrgb各个点的值
        ...
        for j in range(steps):
            for k in range(steps):
                ...
                if tmp.shape[0] > keep_size:
                    # move coordinates back to [0, block_size]     merge.py里面有复原的代码
                    tmp.x -= i * block_size
                    tmp.y -= j * block_size
                    tmp.z -= k * block_size
...
def run(args):
    ...
    ply_files = sorted([f for f in os.listdir(args.input_dir) if '.ply' in f])# ['redandblack_vox10.ply']
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--input_dir',type=str,default='./data/test/forhhy/NP_supplemented/guanyin/', # ./data/test/forhhy/rhetorician/
        help='Directory with point clouds to be partitioned.')
    parser.add_argument(
        '--output_dir',type=str,default='./data/test/forhhy/NP_supplemented/guanyin_block_32/',
